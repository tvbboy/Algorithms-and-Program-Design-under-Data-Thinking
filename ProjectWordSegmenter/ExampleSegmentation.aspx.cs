...
{
    public partial class ExampleSegmentation : System.Web.UI.Page
    {
        ...
        /// <param name="leftToRight">true为从左到右正向分词，false为从右到左反向分词</param>
        ...
        /// <param name="wordList">词典</param>
        /// <returns>储存了分词结果的字符串数组</returns>
        public static List<string> SegMMDouble(string inputStr, ref List<string> wordList, int maxLength)
        {
            ...
            //进行正反向分词
            //正向
            segWordsLeftToRight =SegMM(inputStr, ref wordList, true, maxLength);
            //逆向
            segWordsRightToLeft = SegMM(inputStr, ref wordList, false, maxLength);
            ...
            while ((segWordsLeftToRight[0].Length + segWordsRightToLeft[segWordsRightToLeft.Count - 1].Length) < inputStr.Length)
            {
                ...
                //进行正反向分词
                //正向
                segWordsLeftToRight = SegMM(inputStr, ref wordList, true, 7); 
                //逆向
                segWordsRightToLeft = SegMM(inputStr, ref wordList, false, 7); 
            }
        ...
        protected void btnButtonLeftToRight_Click(object sender, EventArgs e)
        {
            string inputStr = txtInput.Text;
            int maxlength = int.Parse(txtMaxLength.Text);
            List<string> result = SegMM(inputStr, ref wordList, true, maxlength);
            Response.Write("最大匹配算法正向分词的结果:</br>");
        ...
        protected void btnButtonRightToLeft_Click(object sender, EventArgs e)
        {
            string inputStr = txtInput.Text;
            int maxlength = int.Parse(txtMaxLength.Text);
            List<string> result= SegMM(inputStr, ref wordList, false, maxlength);
            Response.Write("最大匹配算法反向分词的结果:</br>");
            foreach (var item in result)
            {
                Response.Write(item+"<br>");
            }
        ...
        protected void btnButtonDouble_Click(object sender, EventArgs e)
        {
            string inputStr = txtInput.Text;
            int maxlength = int.Parse(txtMaxLength.Text);
            List<string> result =SegMMDouble(inputStr, ref wordList, maxlength);
            Response.Write("最大匹配算法双向分词的结果:</br>");
            foreach (var item in result)
            {
                Response.Write(item + "<br>");
            }
